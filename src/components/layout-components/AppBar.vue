  ...
  <div class="app-padding py-3 d-flex align-center" id="app-bar">
    <div class="d-inline-flex align-center w-25" v-show="header">
      <router-link to="/" class="router-link">
        <img
          :src="
            require('@/assets/images/' + this.$store.state.currentTheme + '/logo.svg')
          "
          alt="logo"
          class="app-logo mb-md-n3 ma-0"
    ...
    <div class="d-inline-block text-right w-75" v-show="header">
      <div v-if="!$vuetify.display.mobile" class="d-inline-flex">
        <div
          v-for="(item, i) in nav_link_list"
          :key="i"
          class="d-inline-block align-center"
        >
          <span class="ml-8 d-inline-block">
            <span class="text-dot" @click.prevent="onLinkClick(item.link)">
              {{ item.title }}
            </span>
          </span>
        </div>
...
.text-dot:hover {
  color: rgb(var(--v-theme-stroke-2), 1) !important;
}
...
export default {
  ...
  mounted() {
    ...
    APP_BAR.style.height = this.$vuetify.display.mobile ? "60px" : "80px";
    document.addEventListener(
      "scroll",
      function () {
        if (document.scrollingElement.scrollTop) {
          // if (this.$vuetify.display.mobile)
          //   APP_BAR.style.height =
          //     document.scrollingElement.scrollTop < 50 ? "60px" : "50px";
          // else
          //   APP_BAR.style.height =
          //     document.scrollingElement.scrollTop < 50 ? "130px" : "70px";
          APP_BAR.style.backdropFilter =
            document.scrollingElement.scrollTop > 50 ? "blur(10px)" : "none";
          APP_BAR.style.backgroundColor =
            document.scrollingElement.scrollTop > 50
              ? "rgb(var(--v-theme-surface), 0.6)"
              : "none";
        }
      }.bind(this)
    );
